...
# include <fstream>   // Libreria para poder usar archivos 
...
// En este ejemplo vamos a leer  y a guardar los datos en un archivo de texto
...
// Se necesita una variable para definir el nombre del archivo
// En este ejemplo lo ponemos fijo
char nombre[] = "panaderia.txt";

// Esta rutina lee los datos que están guardados en el archivo y los guarda en los arreglos
// Para que funcione adecuadamente es importante que el archivo si exista y ya tenga al menos 
...
{   
    ...
    // Se inicializa el contador de productos
    total_de_productos = 0;

    // Se procesan todos los registros del archivo y se guardan en los arreglos
    do
    {
        archivo >> tipo[total_de_productos];
        archivo >> modelo[total_de_productos];
        archivo >> tamanio[total_de_productos];
        archivo >> precio[total_de_productos];
        archivo >> existencia[total_de_productos];
        total_de_productos++;
    }while (archivo.eof()!=true);

    // Se cierra el archivo porque ya no se ocupa
    archivo.close();
}
...
// Esta rutina despliega el menú del administrador
...
// Esta función recibe el id y el password a validar y regresa verdadero si se capturaron bien
// antes de la cantidad de intentos que también recibe como parámetro de entrada 
...
void procesosAdmin()
{
   // Declaramos variables de entrada
    ...
    do
    {
        ...
        {
        case 1:
            //Se va a desplegar todo el catálogo, entonces primero se limpia la pantalla
            system("cls");
            // Se indica que la identación debe ser a la izquierda para los encabezados
            cout<< left;
            // Y antes de desplegar cada uno se indica el ancho en el que se debe desplegar
            cout.width(ancho+1);
            cout << "Tipo";
            cout.width(ancho+1);
            cout<<"Modelo";
            ...
            cout.width(ancho+1);
            cout<<"Existencia";
            // Se indica que cambie la identación a la derecha y que brinque de linea
            cout<<right<<endl;
            // Y se despliegan los datos de cada uno de los productos
            ...
            // Se hace una pausa para que el usuario alcance a ver la información
            system("pause");
            break;
        case 2:
            // Se va a agregar un producto al catálogo
            // Entonces hay que pedir todos los datos y guardarlos al final del arreglo
            // No se valida que se pase del tamaño del arreglo ni que se capturen bien los datos
            cout << "Dame el tipo ";
            cin >> tipo[total_de_productos];
            cout << "Dame el modelo ";
            cin >> modelo[total_de_productos];
            cout << "Dame el tamanio ";
            cin >> tamanio[total_de_productos];
            cout << "Dame el precio ";
            cin >> precio[total_de_productos];
            cout << "Dame la existencia ";
...
int main()
{
    ...
    // Primeramente vamos a leer los datos que ya existen en el archivo
    leeDatos();
